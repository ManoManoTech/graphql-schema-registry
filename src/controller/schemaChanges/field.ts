...
export class FieldChangeStrategy implements TypeDefChangeStrategy {
	...
	private operationRepository = OperationTransactionalRepository.getInstance();
	...
	async apply(data: ITypeDefChangeStrategy): Promise<void> {
		...
		const [fieldNames, operationNames] = splits.reduce((acc, cur) => {
		...
		await this.operationRepository.removeOperations(data.trx, operationNames);
	}
}
