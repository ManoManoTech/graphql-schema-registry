...
export function getUsagesForClients(client: ClientPayload, query: any): QueryResult {
	...
	statsWithContext.forEach(stat => {
		...
		const totalErrors = Number(stat.queryLatencyStats.rootErrorStats.errorsCount);
